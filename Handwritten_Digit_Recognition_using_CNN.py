...
def preprocess_image(image):
    ...
    # Note: Humne invert hata diya hai kyunki canvas already black hai
    img_data = np.array(img)
    
    # Center the digit (Cropping logic)
    coords = np.column_stack(np.where(img_data > 0)) 
    if coords.any():  
        top_left = coords.min(axis=0)
        bottom_right = coords.max(axis=0)
        # Thoda padding add karna behtar hota hai
...
class DrawingApp:
    def __init__(self, model): # Fixed: Added underscores
        self.model = model
        self.window = Tk()
    ...
    def predict_digit(self):
        ...
        self.canvas.create_text(140, 30, text=f"Result: {predicted_digit}", fill='red', font=('Helvetica', 20))
